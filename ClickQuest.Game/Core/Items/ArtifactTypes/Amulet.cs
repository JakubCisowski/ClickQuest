...
namespace ClickQuest.Game.Core.Items.ArtifactTypes
{
	public class Amulet : ArtifactTypeFunctionality
	{
		...
		{
			bool hasTwoAccessories = User.Instance.CurrentHero.EquippedArtifacts.Count(x => x.ArtifactType == ArtifactType.Accessory) == 2;
			bool hasAmulet = User.Instance.CurrentHero.EquippedArtifacts.Count(x => x.ArtifactType == ArtifactType.Amulet) == 1;
