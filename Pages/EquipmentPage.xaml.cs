...
namespace ClickQuest.Pages
{
	public partial class EquipmentPage : Page
	{
		...
		public void UpdateEquipment()
		{
			...
			UpdateEquipmentTab<Material>(User.Instance.CurrentHero?.Materials, MaterialsPanel);
			UpdateEquipmentTab<Artifact>(User.Instance.CurrentHero?.Artifacts, ArtifactsPanel);
			UpdateEquipmentTab<Recipe>(User.Instance.CurrentHero?.Recipes, RecipesPanel);
		...
		private void UpdateEquipmentTab <T>(List<T> specificEquipmentCollection, StackPanel equipmentTabPanel) where T : Item
		{
			if (specificEquipmentCollection != null)
			{
				foreach (var item in specificEquipmentCollection)
				{
					...
					var toolTip = TooltipController.GenerateEquipmentItemTooltip<T>(item);
