...
namespace ClickQuest.Extensions.InterfaceManager
{
	public static class TooltipController
	{
		...
		public static ToolTip GenerateCurrencyTooltip<T>(int rarityValue) where T:Item
		{
			...
			var currencyColor = Styles.Colors.GetRarityColor((Rarity)rarityValue);
			currencyTooltipTextBlock.Foreground = currencyColor;

			currencyTooltipTextBlock.Text = ((Rarity)rarityValue).ToString() + " " + (typeof(T)== typeof(Ingot) ? "Ingots" : "Dungeon Keys");
