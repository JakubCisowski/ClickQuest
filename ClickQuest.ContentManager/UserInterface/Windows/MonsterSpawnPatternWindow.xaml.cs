...
namespace ClickQuest.ContentManager.UserInterface.Windows
{
	public partial class MonsterSpawnPatternWindow : Window
	{
		...
		{
			var oldPatternIndex = _region.Monsters.IndexOf(_region.Monsters.FirstOrDefault(x => x.MonsterId == _pattern.MonsterId));
			...
			{
				_region.Monsters.Add(_pattern);
			}
			else
			{
				_region.Monsters[oldPatternIndex] = _pattern;
			}
		}
